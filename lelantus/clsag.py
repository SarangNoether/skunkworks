...
import dumb25519
from dumb25519 import hash_to_scalar, hash_to_point, random_scalar
from common import *
...
def sign(M,p,P,z,C,index=None):
    ...
    if n > 1:
        for i in range(l+1,l+n):
            i = i % n
            L = H2*s[i] + P[i]*(h[i]*mu_P) + C[i]*(h[i]*mu_C)
...
def verify(M,P,C,sig):
    ...
    for i in range(0,n):
        if i == 0:
            ...
            temp_h = h[i]
        L = H2*s[i%n] + P[i%n]*(temp_h*mu_P) + C[i%n]*(temp_h*mu_C)
