...
\item Set $\mu \equiv \hs(\{M_k\},\{P_k\},\{J^{(u)}\})$ (to include all public parameters as well).
\item Define $\{X_j\}_{j=0}^{m-1} \subset \G$ such that: $$X_j \equiv \sumk p_{k,j}\mu^kM_k + \sumu \rho_j^{(u)}G$$
\item Define $\{Y_j\}_{j=0}^{m-1} \subset \G$ such that: $$Y_j \equiv U \sumk p_{k,j}\mu^k + \sumu \rho_j^{(u)}J^{(u)}$$
...
\item Define $\left\{z_R^{(u)}\right\}_{u=0}^{w-1} \subset \F$ such that: $$z_R^{(u)} \equiv \mu^{l^{(u)}}r^{(u)}\xi^m - \sumj \rho_j^{(u)}\xi^j$$
