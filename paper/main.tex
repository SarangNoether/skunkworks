...
%The decisional Diffie-Hellman assumption implies that the elements {Xj} and {Yj} are independent and uniformly distributed in a valid proof" should be changed to "The selection of random \rho values and the computation of each \mu_k and H(M_k) under the random oracle model imply that X, Y, and Z are statistically uniform and independent..." or something like that. DDH is a worse
...
% Since H is a random oracle, knowledge of r implies knowledge of {r^u} except with negligible probability
Collision resistance of $\hs$ means that, with high probability, knowledge of $r$ is equivalent to knowledge of $\{r^{(u)}\}$.
%  it's DL hardness on G together with the random oracle model
